...
def get_params(vocab_size, num_hiddens, device):
    ...
    # ===== 初始化 GRU1 的 3 组门控机制参数 =====
    W_xz1, W_hz1, b_z1 = two_W_one_b()  # 更新门 (Update Gate)
    W_xr1, W_hr1, b_r1 = two_W_one_b()  # 重置门 (Reset Gate)
    W_xh1, W_hh1, b_h1 = two_W_one_b()  # 候选隐藏状态 (Candidate Hidden State)

    # ===== 初始化 GRU1 的输出层参数 =====
    W_hq1 = normal((num_hiddens, num_outputs))  # 隐藏层 -> 输出层
    b_q1 = torch.zeros(num_outputs, device=device)

    # ===== 初始化 GRU2 的 3 组门控机制参数 =====
    W_xz2, W_hz2, b_z2 = two_W_one_b()  # 更新门 (Update Gate)
    W_xr2, W_hr2, b_r2 = two_W_one_b()  # 重置门 (Reset Gate)
    W_xh2, W_hh2, b_h2 = two_W_one_b()  # 候选隐藏状态 (Candidate Hidden State)

    # ===== 初始化 GRU2 的输出层参数 =====
    W_hq2 = normal((num_hiddens, num_outputs))  # 隐藏层 -> 输出层
    b_q2 = torch.zeros(num_outputs, device=device)

    # 隐藏层参数
    # h_t_1 = GRU(X_t,h_{t-1}_1)
    # h_t_2 = GRU(h_t_1,h_{t-1}_2)

    # 附加梯度
    params = [W_xz1, W_hz1, b_z1,
              W_xr1, W_hr1, b_r1,
              W_xh1, W_hh1, b_h1,
              W_hq1, b_q1,
              W_xz2, W_hz2, b_z2,
...
def init_deep_rnn_state(batch_size, num_hiddens, device):
    """
    初始化 RNN 的隐藏状态。
    参数：
...
def deep_rnn(inputs, state, params):
    """
    实现循环神经网络（RNN）的前向传播。
    参数：
    ...
    W_xz1, W_hz1, b_z1 = params[0:3]  # 更新门 (Update Gate)
    W_xr1, W_hr1, b_r1 = params[3:6]  # 重置门 (Reset Gate)
    W_xh1, W_hh1, b_h1 = params[6:9]  # 候选隐藏状态

    W_hq1, b_q1 = params[9:11]  # 输出层参数

    W_xz2, W_hz2, b_z2 = params[11:14]  # 更新门 (Update Gate)
    W_xr2, W_hr2, b_r2 = params[14:17]  # 重置门 (Reset Gate)
    W_xh2, W_hh2, b_h2 = params[17:20]  # 候选隐藏状态

    W_hq2, b_q2 = params[20:22]  # 输出层参数

    H1, H2 = state  # 获取初始隐藏状态
    outputs = []  # 存储所有时间步的输出
    ...
    for X in inputs:
        ...
        # 第一层GRU
        # 1. 更新门 Z_t
        Z1 = torch.sigmoid((X @ W_xz1) + (H1 @ W_hz1) + b_z1)

        # 2. 重置门 R_t
        R1 = torch.sigmoid((X @ W_xr1) + (H1 @ W_hr1) + b_r1)

        # 3. 候选隐藏状态 H_tilda
        H_tilda1 = torch.tanh((X @ W_xh1) + ((R1 * H1) @ W_hh1) + b_h1)

        # 4. 新的隐藏状态 H_t
        H1 = Z1 * H1 + (1 - Z1) * H_tilda1
        ...
        # 第二层GRU
        # 1. 更新门 Z_t
        Z2 = torch.sigmoid((X2 @ W_xz2) + (H2 @ W_hz2) + b_z2)

        # 2. 重置门 R_t
        R2 = torch.sigmoid((X2 @ W_xr2) + (H2 @ W_hr2) + b_r2)

        # 3. 候选隐藏状态 H_tilda
        H_tilda2 = torch.tanh((X2 @ W_xh2) + ((R2 * H2) @ W_hh2) + b_h2)

        # 4. 新的隐藏状态 H_t
        H2 = Z2 * H2 + (1 - Z2) * H_tilda2
